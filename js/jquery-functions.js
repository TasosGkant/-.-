$("document").ready(function () {
  ...
  function skipToEnd(message) {
    ...
    const error =
      currentLanguage === "greek"
        ? "Λυπούμαστε αλλά δεν είναι εφικτή η έκδοση άδειας γάμου."
        : "We are sorry, but it is not possible to issue a marriage license.!";
  ...
  function submitForm() {
    ...
    const titleText =
      currentLanguage === "greek"
        ? "Η έκδοση άδειας γάμου είναι εφικτή!"
        : "Issuing a marriage license possible!";
    ...
    currentLanguage === "greek"
      ? $(".question-container").append(
          "<br /><br /><h5 class='answer'>Τα δικαιολογητικά που πρέπει να προσκομίσετε για να εκδοθεί η άδεια γάμου είναι τα εξής:</h5><br />"
        )
      : $(".question-container").append(
          "<br /><br /><h5 class='answer'>he supporting documents you must provide to issue a marriage license are as follows::</h5><br />"
        );
  ...
  $("#nextQuestion").click(function () {
    if ($(".govgr-radios__input").is(":checked")) {
      ...
      if (currentQuestion === 0 && selectedRadioButtonIndex === 2) {
        currentQuestion = -1;
        currentLanguage === "greek" ? skipToEnd("Πρέπει να έχετε βεβαίωση από την οικεία Προξενική ή άλλη αρμόδια Αρχή.") : skipToEnd("You must have a certificate from the relevant Consular or other competent Authority.");
      } else if (currentQuestion === 1 && selectedRadioButtonIndex === 2) {
        currentQuestion = -1;
        currentLanguage === "greek" ? skipToEnd("Πρέπει να έχετε πιστοποιητικό γέννησης από την χώρα σας μεταφρασμένο στα Ελληνικά.") : skipToEnd("You must have a birth certificate from your country translated into Greek.");
      } else if (currentQuestion === 3 && selectedRadioButtonIndex === 2) {
        currentQuestion = -1;
        currentLanguage === "greek" ? skipToEnd("Πρέπει να έχετε φωτοτυπία Διαβατηρίου ή άδεια διαμονής ή βεβαίωση παραμονής στην Ελλάδα.") : skipToEnd("You must have a photocopy of your Passport or residence permit or certificate of residence in Greece.");
      }else if (currentQuestion === 4 && selectedRadioButtonIndex === 2) {
          currentQuestion = -1;
          currentLanguage === "greek" ? skipToEnd("Πρέπει να βάλετε αγγελία γάμου σε ημερήσια τοπική εφημερίδα.") : skipToEnd("You must place a marriage advertisement in a daily local newspaper.");
       } else if (currentQuestion === 5 && selectedRadioButtonIndex === 2) {
            currentQuestion = -1;
            currentLanguage === "greek" ? skipToEnd("Πρέπει να έχετε φωτοτυπία Διαβατηρίου ή πιστοποιητικό γέννησης του/ης μέλλοντος συζήγου σας.") : skipToEnd("You must have a photocopy of your future spouse's passport or birth certificate.");
        } else {
